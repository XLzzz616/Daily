...
# -*- encoding: utf-8 -*-
'''
@File         :bdqx_qjsj_v1.py
@Time         :2020/04/24 10:10:21
@Auther       :Xuz
@Version      :1.0
@Notes        :下载百度迁徙-分市时间段区间数据
              (三个区间：1：0110-至今；2:0110-0124；3:0125-至今)
              注：因区间在变化，该类数据无法下载历史数据
'''
...
def getData_Scale(tarUrl,qjDate,curDate):    #针对规模指数进行求和计算，返回结果
    periodPartUrl=tarUrl    
    patDate2020 = '"(2020.*?)":\d+\.*\d.*?\D'
    patData2020 = '"2020.*?":(\d+\.*\d.*?)\D'
    pat2020List=[patDate2020,patData2020]
    res2020=getData(periodPartUrl,pat2020List)
    date2020=res2020[0]
    data2020=res2020[1]
    downDate='20200110'
    if qjDate == 1:
        ...
        upDate=curDate
    totalScale=0
    for i9 in range(0,len(date2020)):
        if (int(date2020[i9])>=int(downDate)) and (int(date2020[i9])<=int(upDate)):
            totalScale=totalScale+(float(data2020[i9])*10000)
    return totalScale


def everyData_Period(pFilename,date,cTarget,cDate): #存储路径，数据区间号，给出的流向城市数目,当前日期
    f = xlwt.Workbook()
    sheet1 = f.add_sheet(u'2020年分市迁徙流向', cell_overwrite_ok=True)    
    row0 = [u'所在城市',u'迁入来源地',u'比例',u'省份',u'总迁入规模',u'所在城市',u'迁出目的地',u'比例',u'省份',u'总迁出规模']
    # 写入首行
    ...
    for i in range(0,len(ID)):
        ...
        column4=getData_Scale(moveInScaleurl,tDate,cDate)
        ...
        column24=getData_Scale(moveOutScaleurl,tDate,cDate)
...
if __name__=='__main__':
    ...
    for i in dateClass:
        ...
        logPath='F:/DataGet/BDqianxi/'+'logTotal'+str(i)+'_'+str(dataCurrent)+'.txt'
        list1,list2=everyData_Period(fileName,i,100,dataCurrent)
